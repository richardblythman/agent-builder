...
1. **Update `.claude/commands/README.md`**:
   - Read the existing README.md in `.claude/commands/`
   - Add a new bullet point for your command with format:
     `- [command-name.md](./command-name.md) - Brief description of what it does`
   - This ensures the command index stays in sync and all links remain valid

2. **Why this matters**:
   - Every time a new command is created, the command index is automatically updated
   - This keeps the navigation hub current and ensures developers and Claude can easily discover all commands
   - Prevents broken links and missing commands from the index
